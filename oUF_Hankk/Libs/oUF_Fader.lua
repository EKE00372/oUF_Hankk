-- Adapted from oUF_Ruri's oUF_Fader.
-- Source: https://github.com/EKE00372/oUF_Ruri/blob/dac8e7794a0e844daabf2adae9d03bcef1ee0749/Libs/oUF_Fader.lua
-- License: see oUF_Fader.LICENSE.txt.
-- Changes: use the embedded Hankk namespace and a Hankk-owned hook marker.

local _, ns = ...
local C = ns[1]
local oUF = assert(ns.oUF, "oUF_Hankk: embedded oUF must load before oUF_Fader")
...
local function Enable(self, unit)
	...
	if not self.__hankkFaderHooks then
		self:HookScript("OnEnter", Update)
		self:HookScript("OnLeave", Update)
		self.__hankkFaderHooks = true
	end
